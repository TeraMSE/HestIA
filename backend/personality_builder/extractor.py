...
class PersonalityExtractor:
    ...
    def extract_evidence_quotes(
        ...
        system_prompt = (
            ...
            "evidence. Be precise and only extract what was explicitly stated or strongly implied."
        )
        user_message = (
        ...
        result = self.llm_client.complete_structured(
            ...
            output_schema={
                "evidence_items": [
                    {"quote": "string", "trait": "string", "implied_value": 0.0, "confidence": 0.0, "context": "string"}
                ]
            },
            use_fast_model=True,
        )
        ...
        for raw_item in result.get("evidence_items", []) or []:
            ...
            implied_value = max(0.0, min(1.0, self._as_float(raw_item.get("implied_value"), 0.5)))
            confidence = max(0.0, min(1.0, self._as_float(raw_item.get("confidence"), 0.5)))
            ...
            evidence_items.append({"quote": quote, "trait": trait, "implied_value": implied_value, "confidence": confidence, "context": context})

        time.sleep(0.5)
        return evidence_items

    def extract_holistic_bigfive(self, transcript: List[Dict[str, str]]) -> Dict[str, Any]:
        user_text = self._user_text_block(transcript)
        if not user_text:
            return self._empty_holistic_result()

        system_prompt = (
            "You are an expert psychologist trained in the Big Five (OCEAN) personality model. "
            "You assess personality from written text using established psychometric principles."
        )
        user_message = (
            "Analyze this person's self-description and infer their Big Five personality traits.\n\n"
            f"Text:\n---\n{user_text}\n---\n\n"
            ...
            "Base your assessment ONLY on what was explicitly stated or strongly implied."
        )
    ...
    def extract_full(
        self, transcript: List[Dict[str, str]], graph: PersonalityKnowledgeGraph
    ) -> Dict[str, Any]:
        ...
        for dimension, payload in holistic.items():
            if dimension not in self.SUPPORTED_TRAITS:
                continue
            implied_value = (
                1.0 if bool(payload.get("value", False)) else 0.0
                if dimension == "smoker"
                else self._as_float(payload.get("score"), 0.5)
            )
            confidence = self._as_float(payload.get("confidence"), 0.5)
            justification = str(payload.get("justification", "")).strip() or f"Holistic inference for {dimension}."
            graph.add_evidence(
        ...
        for trait_name in sorted(self.SUPPORTED_TRAITS):
            try:
                confidence_per_trait[trait_name] = float(graph.get_trait_value(trait_name).get("confidence", 0.0))
            except ValueError:
                confidence_per_trait[trait_name] = 0.0

        return {
            "trait_vector": trait_vector,
            "bigfive_raw": {t: holistic.get(t, {}) for t in self.OCEAN_TRAITS},
            "evidence_count": sum(1 for _, d in graph.graph.nodes(data=True) if d.get("type") == "evidence"),
    ...
    def _empty_holistic_result(self) -> Dict[str, Any]:
        empty = {t: {"score": 0.5, "justification": "No user evidence.", "confidence": 0.0} for t in self.OCEAN_TRAITS}
        empty.update({
            "noise_sensitivity": {"score": 0.5, "justification": "No user evidence.", "confidence": 0.0},
            "thermal_sensitivity": {"score": 0.5, "justification": "No user evidence.", "confidence": 0.0},
            "cleanliness": {"score": 0.5, "justification": "No user evidence.", "confidence": 0.0},
            "schedule_preference": {"score": 0.5, "justification": "No user evidence.", "confidence": 0.0},
            "smoker": {"value": False, "justification": "No explicit mention.", "confidence": 0.0},
        })
        return empty

    def _normalize_holistic(self, raw: Dict[str, Any]) -> Dict[str, Any]:
        normalized = self._empty_holistic_result()
        for trait in self.OCEAN_TRAITS + ["noise_sensitivity", "thermal_sensitivity", "cleanliness", "schedule_preference"]:
            payload = raw.get(trait, {}) if isinstance(raw, dict) else {}
            normalized[trait] = {
                "score": max(0.0, min(1.0, self._as_float(payload.get("score"), 0.5))),
                "justification": str(payload.get("justification", "")).strip() or f"Inferred {trait}.",
        ...
        normalized["smoker"] = {
            ...
            "justification": str(smoker_payload.get("justification", "")).strip() or "Smoker status inferred only from explicit mention.",
            "confidence": max(0.0, min(1.0, self._as_float(smoker_payload.get("confidence"), 0.5))),
        }
        return normalized

    @staticmethod
    def _smoker_explicit_signal(text: str):
        lowered = (text or "").lower()
        if "smok" not in lowered:
            return None
        if any(t in lowered for t in ["don't smoke", "do not smoke", "not a smoker", "non-smoker", "never smoke"]):
            return False
        if any(t in lowered for t in ["i smoke", "smoke occasionally", "i do smoke", "i am a smoker"]):
            return True
        return None
