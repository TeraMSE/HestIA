...
from .room_layout_model import HorizonNet
...
def run_inference_on_image(pth: Path, img_path: Path, output_dir: Path,
                           force_cuboid: bool = False) -> Path:
    """
    Load HorizonNet from checkpoint, run inference on a single image,
    ...
    img_ori = np.array(img_pil.convert('RGB'))[..., :3].transpose([2, 0, 1]).copy()
    x = torch.FloatTensor([img_ori / 255.0])

    # Run inference
    with torch.no_grad():
        cor_id, z0, z1, _ = inference(
            net=net, x=x, device=device,
            flip=False, rotate=[],
            visualize=False,
