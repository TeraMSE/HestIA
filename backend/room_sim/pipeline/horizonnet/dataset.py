...
class PanoCorBonDataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # Manual Manhattan distance calculation (p=1) to avoid scipy.spatial.distance.cdist issues
        width_arange = np.arange(img.shape[1], dtype=np.float32)
        dist_o = np.abs(corx.reshape(-1, 1) - width_arange.reshape(1, -1))
        dist_r = np.abs(corx.reshape(-1, 1) - (width_arange.reshape(1, -1) + img.shape[1]))
        dist_l = np.abs(corx.reshape(-1, 1) - (width_arange.reshape(1, -1) - img.shape[1]))
